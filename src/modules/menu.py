...
class LevelMenu(Menu):
    ...
    def movePointerQuit(self):
        """
        Method that handles logic of quitting the game prompt.
        """
        if self.saveMonit == '':
            self.saveMonit = 'Yes'
    ...
    def monitCheckInput(self, width, height, sprites, timer):
        """
        Method that handles prompt for quitting the game and saving the board.
        """
        self.game.check_events()
...
class InputName(Menu):
    ...
    def check_input(self):
        ...
        if len(self.game.playerName) > 0 and self.game.START_KEY:
            if self.game.previousState == 'Start':
                self.game.currentMenu = self.game.diffMenu
                self.game.running = True
                self.blit_screen()
                self.game.currentMenu.display_menu()

            elif self.game.previousState == 'Level':
                self.game.currentMenu = self.game.levelMenu
                self.game.running = True
                self.blit_screen()
                self.game.currentMenu.display_menu()
